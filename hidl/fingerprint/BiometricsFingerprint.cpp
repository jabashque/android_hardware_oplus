...
Return<RequestStatus> BiometricsFingerprint::authenticate(uint64_t operationId, uint32_t gid) {
    // Just in case postEnroll never got called for whatever reason,
    // e.g. locked the screen while enrolling fingerprints.
    if (mIsEnrolling) {
        mIsEnrolling = false;
        setDimlayerHbm(0);
    }
...
Return<void> BiometricsFingerprint::onError(uint64_t deviceId, FingerprintError error,
                                            int32_t vendorCode) {
    setFpPress(0);
    if (error != FingerprintError::ERROR_CANCELED) {
        setDimlayerHbm(0);
    }
